...
#             optimizer.step()
        ...
        out = simba.simba_batch(X[0], torch.Tensor([1]).repeat(len(X[0])).long().cuda(), 1000, 64, 8, 4/255, pixel_attack = True)
...
class TrackSequencesClassifier(object):
    ...
    def classifyn(self, track_sequences, pert, start_idx):
        ...
        # print(out)
        out = simba.simba_batch(rsf(track_sequences), torch.Tensor([1]).repeat(len(track_sequences)).long().cuda(), 500, 192, 12, 14/255, pixel_attack = True)
        advs = out[0]
        l2s = out[4]
