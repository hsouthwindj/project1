...
def save_image(img, filename): # not working
    ...
    inv_image = invTrans(img)
    inv_image = torch.mul(inv_image, 255)
    inv_image = inv_image.long()
    inv_image = np.transpose(inv_image.cpu().detach().numpy(), (1,2,0))
    inv_image = cv2.convertScaleAbs(inv_image)
    inv_image = cv2.cvtColor(inv_image, cv2.COLOR_BGR2RGB)
    cv2.imwrite(filename, inv_image)
...
data_path = '/notebooks/atk/input/cur'
...
def rnnatk(l3, sec_phase, max_iters, full_pert, reg_type):
    ct = time.time()
    for vid_name, (data, y) in video_loader(data_path):
        logging.info('new video %s', vid_name)
        X = data.to(device)
        ...
        while it < maxiter:
            ...
            if l3 == True:
                loss = 0.5 * loss1 + weight_loss2 * loss2 + 1.3 * loss3# + 0.8 * reg
